...
function AppCtrl(
  ...
  $scope.$watch("showMap", function () {
    ...
    if ($rootScope.showMap) $scope.mapContainerStyle = {};
    else $scope.mapContainerStyle = { position: "absolute", left: "-10000px" };
  });
  ...
  $scope.updateSuggestions = function () {
    ...
    if ($scope.q && $scope.q.length > 0) {
      Place.suggestions(
        { field: "nameSuggestions", text: $scope.q, queryId: $scope.queryId },
        function (result) {
          if (
            result.suggestions &&
            result.suggestions.length > 0 &&
            result.queryId[0] == $scope.queryId
          )
  ...
  $scope.submit = function () {
    ...
    $location
      .path("/search")
      .search({ q: EscapingService.escape($scope.q), type: $scope.type });
    $scope.q = null;
  };
  ...
  $scope.setHighlight = function (id, type, index) {
    if (id == null) $scope.highlight = null;
    else $scope.highlight = { id: id, type: type, index: index };
  };
}
...
function HomeCtrl($scope, $location, $rootScope, Place, EscapingService) {
    ...
    if ($scope.homeSearchSuggestions.length > 0) {
      ...
      if ($scope.selectedSuggestionIndex < 0)
        $scope.selectedSuggestionIndex =
          $scope.homeSearchSuggestions.length - 1;
    }
  };
  ...
  $scope.submit = function () {
    if (
      $scope.selectedSuggestionIndex != -1 &&
      $scope.homeSearchSuggestions.length > 0
    )
      $scope.searchFieldInput =
        $scope.homeSearchSuggestions[$scope.selectedSuggestionIndex];
    $scope.zoom = 2;
    $location
      .path("/search")
      .search({
        q: EscapingService.escape($scope.searchFieldInput),
        type: $scope.type,
      });
...
function ExtendedSearchCtrl(
  ...
  $scope.submit = function () {
          ...
          should: [
            {
              nested: {
                path: "names",
                query: { match: { "names.language": $scope.names.language } },
              },
            },
            { match: { "prefName.language": $scope.names.language } },
          ],
    ...
    if ($scope.parent !== null) {
      ...
      } else queries.push({ match: { parent: $scope.parent.gazId } });
    }
    ...
    if (
      ...
      $scope.groupInternalData.text &&
      $scope.groupInternalData.groupId
    ) {
      queries.push({
        nested: {
          path: "groupInternalData",
          query: {
            bool: {
              must: [
                {
                  match: {
                    "groupInternalData.text": $scope.groupInternalData.text,
                  },
                },
                {
                  match: {
                    "groupInternalData.groupId":
                      $scope.groupInternalData.groupId,
                  },
                },
              ],
    ...
    if ($scope.ids.value !== "" && $scope.ids.context !== "") {
          ...
          query: {
            bool: {
              must: [
                {
                  match: {
                    "ids.value": { query: $scope.ids.value, operator: "and" },
                  },
                },
      ...
      queries.push({
        nested: {
          path: "ids",
          query: { match: { "ids.value": $scope.ids.value } },
        },
      });
      ...
      queries.push({
        nested: {
          path: "ids",
          query: { match: { "ids.context": $scope.ids.context } },
        },
      });
    }
    ...
    // filters
    var filterQuery = "";
    if ($scope.filters.unlocatable) filterQuery += "unlocatable: true";
    else {
    ...
    $location
      .path("/search")
      .search({ q: angular.toJson(query), fq: filterQuery, type: "extended" });
  };
}
...
function SearchCtrl(
  ...
  $scope.$watch("places", function () {
    for (var i in $scope.places) $scope.places[i].mapType = "searchResults";
    $rootScope.activePlaces = $scope.places;

    if (
      $scope.places.length == 0 &&
      $scope.search.limit + $scope.search.offset > 10000
    ) {
      $rootScope.addAlert(
        messages["ui.search.result-limit-exceeded"],
        null,
        "warning",
      );
    }
  ...
  $scope.totalPages = function () {
    var totalPages =
      ($scope.total - ($scope.total % $scope.search.limit)) /
      $scope.search.limit;
  ...
  $scope.prevFacetEntries = function (facetName) {
    ...
    if ($scope.facetOffsets[facetName] < 0) $scope.facetOffsets[facetName] = 0;
  };
    ...
    Place.query(
      ...
      function (result) {
        ...
        for (var i in $scope.places) {
          if (
            $scope.places[i].prefLocation &&
            !$scope.places[i].prefLocation.shape &&
            $scope.places[i].prefLocation.coordinates &&
            $scope.places[i].prefLocation.coordinates.length > 0
          ) {
            markers++;
        ...
        $rootScope.loading--;
        if ($scope.total != result.total) $scope.total = result.total;
      },
      function () {
        $rootScope.addAlert(
          messages["ui.contactAdmin"],
          messages["ui.error"],
          "error",
        );
        $rootScope.loading--;
  ...
  $scope.setFacet = function (facetName, term) {
    if ($scope.search.type == "extended") {
      ...
      if ($scope.search.fq)
        $scope.search.fq += " AND " + facetName + ':"' + term + '"';
  ...
  $scope.$watch("filters.coordinates", function () {
    $scope.changeFilters();
  });

  $scope.$watch("filters.noCoordinates", function () {
    $scope.changeFilters();
  });

  $scope.$watch("filters.polygon", function () {
    $scope.changeFilters();
  });

  $scope.$watch("filters.noPolygon", function () {
    $scope.changeFilters();
  });

  $scope.$watch("filters.unlocatable", function () {
    $scope.changeFilters();
  });
  ...
  $scope.updateFilters = function () {
    ...
    if ($scope.search.fq) {
      $scope.search.fq = $scope.search.fq.replace(
        " AND NOT _exists_:prefLocation.coordinates",
        "",
      );
      $scope.search.fq = $scope.search.fq.replace(
        " AND _exists_:prefLocation.coordinates",
        "",
      );
      $scope.search.fq = $scope.search.fq.replace(
        "NOT _exists_:prefLocation.coordinates",
        "",
      );
      $scope.search.fq = $scope.search.fq.replace(
        "_exists_:prefLocation.coordinates",
        "",
      );
      $scope.search.fq = $scope.search.fq.replace(
        " AND NOT _exists_:prefLocation.shape",
        "",
      );
      $scope.search.fq = $scope.search.fq.replace(
        " AND _exists_:prefLocation.shape",
        "",
      );
      $scope.search.fq = $scope.search.fq.replace(
        "NOT _exists_:prefLocation.shape",
        "",
      );
      $scope.search.fq = $scope.search.fq.replace(
        "_exists_:prefLocation.shape",
        "",
      );
    ...
    } else $scope.search.fq = filterQuery;
  };
  ...
  function setSearchFromLocation() {
    ...
    if ($location.search().order)
      $scope.search.order = $location.search().order;
    if ($location.search().bbox) $scope.search.bbox = $location.search().bbox;
    if ($location.search().polygonFilterCoordinates)
      $scope.search.polygonFilterCoordinates =
        $location.search().polygonFilterCoordinates;
    if ($location.search().showInReview)
      $scope.search.showInReview = $location.search().showInReview;
    ...
    if ($scope.search.fq) {
      if ($scope.search.fq.indexOf("unlocatable: true") > -1)
        ...
        if (
          $scope.search.fq.indexOf("_exists_:prefLocation.coordinates") > -1
        ) {
          if (
            $scope.search.fq.indexOf("NOT _exists_:prefLocation.coordinates") >
            -1
          )
            $scope.filters.noCoordinates = true;
          else $scope.filters.coordinates = true;
        }
        if ($scope.search.fq.indexOf("_exists_:prefLocation.shape") > -1) {
          ...
          else $scope.filters.polygon = true;
        }
      }
    }
  }
...
function CreateCtrl(
  ...
  $scope.addPlaceType = function (placeType) {
    ...
    if (pos == -1) {
      if (!$scope.place.types) $scope.place.types = [];
      $scope.place.types.push(placeType);
    } else $scope.place.types.splice(pos, 1);
  };
  ...
  $scope.hasType = function (placeType) {
    if (!$scope.place || !$scope.place.types) return false;

    if ($scope.getListPos($scope.place.types, placeType) != -1) return true;
    else return false;
  };
  ...
  $scope.save = function () {
    ...
    if ($scope.place.recordGroup == {}) $scope.place.recordGroup = undefined;
    ...
    Place.save(
      ...
      function (data) {
        if (data.message == null) {
          ...
          $rootScope.addAlert(
            messages["ui.place.save.success"],
            null,
            "success",
          );
        } else {
          ...
          $rootScope.addAlert(
            messages["ui.place.save.failure." + $scope.failure],
            null,
            "error",
          );
          window.scrollTo(0, 0);
...
function PlaceCtrl(
    ...
    $scope.place = Place.get(
      ...
      function (result) {
        ...
        if ($scope.place.accessDenied) {
          ...
          $rootScope.pageTitle =
            messages["ui.place.hiddenPlace"] + " | iDAI.gazetteer";
        }
        if ($scope.place.replacing)
          $rootScope.addAlert(
            messages["ui.place.replacing.first"] +
              $scope.place.replacing +
              messages["ui.place.replacing.second"],
            null,
            "warning",
          );
        ...
        if (!$scope.place.recordGroup) $scope.place.recordGroup = { id: "" };
        $scope.originalRecordGroupId = $scope.place.recordGroup.id;
        $rootScope.loading++;
        Place.query(
          {
            q: "relatedPlaces:" + $scope.place.gazId,
            sort: "prefName.title.sort",
            showHiddenPlaces: true,
            add: "sort",
          },
          function (result) {
      ...
      function (response) {
        if (response.status == 403) {
          $scope.place = {
            gazId: $routeParams.id,
            recordGroup: response.data.recordGroup,
            accessDenied: true,
          };
          $rootScope.title = messages["ui.place.hiddenPlace"];
          $rootScope.pageTitle =
  ...
  $scope.$watch("place.prefLocation.coordinates", function () {
    if ($scope.place && $scope.place.prefLocation) $scope.updateMap();
  });
  ...
  $scope.$watch("place.gazId", function () {
    if ($scope.place && $scope.place["@id"])
      $rootScope.subtitle =
        $scope.place["@id"] +
        ' <a data-toggle="modal" href="#copyUriModal"><i class="icon-share" style="font-size:0.7em"></i></a>';
  });
  ...
  $scope.updateMap = function () {
    ...
    for (var i in $scope.place.parents) {
      if (
        (!$scope.place.prefLocation || !$scope.place.prefLocation.shape) &&
        $scope.place.parents[i].prefLocation &&
        $scope.place.parents[i].prefLocation.shape &&
        !polygonPlace
      )
  ...
  $scope.changeNumberOfDisplayedNames = function () {
    if ($scope.namesDisplayed == 4) $scope.namesDisplayed = 10000;
    else $scope.namesDisplayed = 4;
  };

  $scope.changeNumberOfDisplayedIds = function () {
    if ($scope.idsDisplayed == 3) $scope.idsDisplayed = 10000;
    else $scope.idsDisplayed = 3;
  };

  $scope.changeNumberOfDisplayedLinks = function () {
    if ($scope.linksDisplayed == 3) $scope.linksDisplayed = 10000;
    else $scope.linksDisplayed = 3;
  };
    ...
    Place.query(
      {
        q: "relatedPlaces:" + $scope.place.gazId,
        sort: "prefName.title.sort",
        offset: $scope.offsetRelatedPlaces,
        showHiddenPlaces: true,
        add: "sort",
      },
      function (result) {
    ...
    Place.duplicate(
      ...
      function (data) {
        if (data.message == null) {
          ...
          $rootScope.addAlert(
            messages["ui.place.duplicate.success"],
            null,
            "success",
          );
        } else {
          ...
          $scope.failure = data.message;
          $rootScope.addAlert(
            messages["ui.place.duplicate.failure." + $scope.failure],
            null,
            "error",
          );
          window.scrollTo(0, 0);
      ...
      function (result) {
        ...
        $scope.failure = result.data.message;
        $rootScope.addAlert(
          messages["ui.place.duplicate.failure"],
          null,
          "error",
        );
        window.scrollTo(0, 0);
  ...
  $scope.prepareSave = function () {
    if ($scope.place.unlocatable) {
      ...
      $scope.place.prefLocation &&
      (!$scope.place.prefLocation.coordinates ||
        $scope.place.prefLocation.coordinates.length == 0) &&
  ...
  $scope.addComment = function () {
    ...
    if ($scope.place.comments == undefined) $scope.place.comments = [];
  ...
  $scope.addGroupInternalData = function () {
    ...
    newData.recordGroup.id = $scope.groupInternalData.group.substr(
      0,
      $scope.groupInternalData.group.indexOf("$$"),
    );
    newData.recordGroup.name = $scope.groupInternalData.group.substr(
      $scope.groupInternalData.group.indexOf("$$") + 2,
    );
  ...
  $scope.addName = function () {
    ...
    if ($scope.place.names == undefined) $scope.place.names = [];
  ...
  $scope.removeName = function (name) {
    ...
    if (index > -1) $scope.place.names.splice(index, 1);
  };

  $scope.addLocation = function () {
    if (
      $scope.location.shape &&
      $scope.location.shape.coordinates &&
      $scope.location.shape.coordinates.length == 0
    )
      $scope.location.shape = null;

    if (
      (!$scope.location.coordinates ||
        $scope.location.coordinates.length == 0) &&
      !$scope.location.shape
    )
      return;
    if ($scope.place.locations == undefined) $scope.place.locations = [];
    $scope.place.locations.push($scope.location);
    if ($scope.hasType("archaeological-site"))
      $scope.location = { confidence: 0, publicSite: false, coordinates: [] };
    else $scope.location = { confidence: 0, publicSite: true, coordinates: [] };
  };

  $scope.addIdentifier = function () {
    if (!$scope.identifier.value || !$scope.identifier.context) return;
    if ($scope.place.identifiers == undefined) $scope.place.identifiers = [];
    ...
    if (contextIndex > -1) {
      ...
      if ($scope.place.links == undefined) $scope.place.links = [];
  ...
  $scope.addRelatedPlace = function () {
    ...
    if (relatedPlaces == undefined) relatedPlaces = [];
  ...
  $scope.getIdsByContext = function (context) {
    ...
    for (var i in ids) if (ids[i].context == context) result.push(ids[i].value);
    return result;
  };
  ...
  var removeChildrenFromMap = function () {
    ...
    for (var i in places) {
      if (
        places[i].mapType == "markerChild" ||
        places[i].mapType == "markerChildInvisible"
      )
  ...
  $scope.showChildMarker = function (child) {
    ...
    for (var i in $scope.children) {
      ...
      else $scope.children[i].mapType = "markerChild";
  ...
  $scope.showPolygon = function (place) {
    ...
    if (place != $scope.place) {
      if (place != $scope.mainMapPolygonPlace) place.mapType = "polygonParent";
      else place.mapType = "mainPolygon";
    }
    places.push(place);
  ...
  $scope.recordGroupSearch = function () {
    $timeout(function () {
      $location
        .path("/search")
        .search({ q: "recordGroupId:" + $scope.place.recordGroup.id });
    }, 200);
  };
  ...
  $scope.sendRecordGroupContactMail = function () {
    $http
      .post(
        "../sendRecordGroupContactMail?groupId=" + $scope.place.recordGroup.id,
        $scope.contactMessage,
      )
      .success(function (data, status) {
        $rootScope.addAlert(
          messages["ui.recordGroupContact.success"],
          null,
          "success",
        );
      })
      .error(function (data, status) {
        $rootScope.addAlert(
          messages["ui.recordGroupContact.error"],
          null,
          "error",
        );
      });
...
function MergeCtrl(
  ...
  $scope.getCandidatesByName = function () {
    var query =
      '(prefName.title:"' +
      EscapingService.escape($scope.place.prefName.title) +
      '"~0.5' +
      ' OR names.title:"' +
      EscapingService.escape($scope.place.prefName.title) +
      '"~0.5';
    for (var i in $scope.place.names) {
      query +=
        ' OR prefName.title:"' +
        EscapingService.escape($scope.place.names[i].title) +
        '"~0.5' +
        ' OR names.title:"' +
        EscapingService.escape($scope.place.names[i].title) +
        '"~0.5';
    ...
    $rootScope.loading++;
    Place.query(
      { q: query, type: "queryString", add: "access,sort" },
      function (result) {
        $scope.candidatePlaces = result.result;
        $scope.parents = {};
        for (var i in $scope.candidatePlaces) {
          if (
            $scope.candidatePlaces[i].parent &&
            !$scope.parents[$scope.candidatePlaces[i].parent]
          ) {
            var parentId = getIdFromUri($scope.candidatePlaces[i].parent);
            $scope.parents[$scope.candidatePlaces[i].parent] = Place.get({
              id: parentId,
            });
          }
        }
  ...
  $scope.link = function (place1, place2) {
    if (place1.relatedPlaces == undefined) place1.relatedPlaces = [];
    ...
    Place.save(
      ...
      function (result) {
        $rootScope.addAlert(
          messages["ui.place.save.failure"],
          place1.prefName.title,
          "error",
        );
        $rootScope.loading--;
    ...
    if (place2.relatedPlaces == undefined) place2.relatedPlaces = [];
  ...
  $scope.merge = function (place1, place2) {
    ...
    place1.$merge(
      ...
      function (result) {
        $rootScope.addAlert(
          messages["ui.merge.success.body"],
          messages["ui.merge.success.head"],
          "success",
        );
        $rootScope.loading--;
...
function ThesaurusCtrl($scope, $rootScope, $location, Place, messages, $route) {
  ...
  $scope.showMarker = function (place) {
    ...
    if (!polygonPlace) $rootScope.activePlaces = [place];
    else {
      polygonPlace.mapType = "polygonParent";
  ...
  var getPolygonPlace = function (place, isParent) {
    if (isParent && place.prefLocation && place.prefLocation.shape)
      return place;
    else if (place.parentPlace) return getPolygonPlace(place.parentPlace, true);
    return null;
  };
}
...
function HelpCtrl(
  ...
  $scope.baseUri = $location
    .absUrl()
    .substring(0, $location.absUrl().indexOf("app"));
  $scope.headlines = [];
  ...
  $scope.changeText = function (language, loginNeeded) {
    ...
    $http({
      ...
      .success(function (result) {
        $rootScope.loading--;
        $rootScope.addAlert(
          messages["ui.help.editor.success"],
          null,
          "success",
        );
      })
  ...
  $scope.showPreview = function (language, loginNeeded) {
    $scope.previewText = markdownToHtml(
      $scope.helpTexts[language][loginNeeded],
    );
  };
  ...
  var loadHelpText = function (language, loginNeeded, showRestoreMessage) {
    $http
      .get($scope.baseUri + "help/" + language + "/" + loginNeeded)
      .then(function (result) {
        $scope.helpTexts[language][loginNeeded] = result.data;
        if (showRestoreMessage)
          $rootScope.addAlert(
            messages["ui.help.editor.restored"],
            null,
            "alert",
          );
      });
  ...
  var markdownToHtml = function (markdown) {
    ...
    html = html
      .split("%YOUTUBE=")
      .join('<iframe height="315" width="600" src="');
    html = html
      .split("%!")
      .join('" frameborder="0" allowfullscreen>Youtube-Video</iframe>');
